...
const DEFAULT_COMPS = ["PL","CL","FAC","EL","ECL","WC","EC"];
...
exports.handler = async (event) => {
  try {
    ...
    // Fetch each competition matches for date range
    const results = [];
    for (const comp of compList) {
      ...
      if (!res.ok) {
        ...
        results.push({ __error:true, comp, status: res.status, body: text.slice(0,400) });
        continue;
      }
